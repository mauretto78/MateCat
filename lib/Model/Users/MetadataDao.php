...
class MetadataDao extends \DataAccess_AbstractDao {
     ...
     * @return mixed
     */
    public function getUidByDqfUsernameAndPassword( $username, $password ) {
        $conn = Database::obtain()->getConnection();
        $stmt = $conn->prepare(
                "SELECT a.uid
                FROM user_metadata a
                LEFT OUTER JOIN user_metadata b ON a.uid = b.uid AND b.value = :password
                WHERE 
                a.value = :username
